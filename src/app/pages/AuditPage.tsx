...
import { fetchAllProfiles, fetchEvidenceForProfile, reviewEvidence } from '../../services/taskService';
...
export function AuditPage() {
  ...
  async function handleGetUrl(evidenceId: string, filePath: string) {
    if (signedUrls[evidenceId]) {
      window.open(signedUrls[evidenceId], '_blank');
      return;
    }
    ...
    try {
      ...
      alert('No se pudo generar el enlace. Verifica permisos del bucket.');
    } finally {
      setLoadingUrl(null);
    }
  }
      ...
      <section className="grid gap-4 md:grid-cols-3 mb-6">
        <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5 p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-yellow-400 font-medium">Pendientes</span>
        ...
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-emerald-400 font-medium">Aprobadas</span>
        ...
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-red-400 font-medium">Rechazadas</span>
              ...
              return (
                <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5 flex flex-col gap-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`shrink-0 rounded-full border px-2 py-0.5 text-xs font-medium ${cfg.color}`}>
                      {cfg.label}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 text-xs text-slate-500">
                    ...
                    {task?.phase && (
                      <span className="rounded-lg border border-slate-700 px-2 py-1">{task.phase}</span>
                    )}
                  </div>
                    ...
                    {item.status === 'pending' && (
                      <>
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-lg bg-emerald-500 px-3 py-1.5 text-xs font-medium text-slate-950 hover:bg-emerald-400 disabled:opacity-50"
                        >
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-lg border border-red-500/40 px-3 py-1.5 text-xs text-red-300 hover:bg-red-500/10 disabled:opacity-50"
                        >
