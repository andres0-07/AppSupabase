...
import { createWeeklyGoal } from '../../services/weeklyGoalService';
...
export function CEODashboardPage() {
  ...
  const [goalForm, setGoalForm]         = useState({ weekLabel: '', content: '', assignedTo: '' });
  ...
  async function handleSaveGoal() {
    if (!profile || !goalForm.weekLabel.trim() || !goalForm.content.trim()) return;
    setSavingGoal(true);
    try {
      await createWeeklyGoal({
        weekLabel:  goalForm.weekLabel.trim(),
        content:    goalForm.content.trim(),
        assignedTo: goalForm.assignedTo || null,
        createdBy:  profile.id,
      });
      setShowGoalForm(false);
      setGoalForm({ weekLabel: '', content: '', assignedTo: '' });
    } finally {
      setSavingGoal(false);
    }
  }
            ...
            {dependencyAlerts.map((task) => {
              ...
              return (
                <div key={task.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/20 bg-slate-950/60 p-3">
                  ...
                  <button
                    onClick={() => { updateTask(task.id, { status: 'in_progress' }); loadData(); }}
                    className="rounded-full bg-amber-500/20 px-2 py-1 text-xs text-amber-400 hover:bg-amber-500/40 border border-amber-500/30"
                  >
                    ⚡ Urgente iniciar
                  </button>
                </div>
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-violet-500/40 px-4 py-2 text-sm font-medium text-violet-300 hover:bg-violet-500/10"
              >
                🎯 Objetivo semanal
              </button>
      ...
      {showGoalForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-violet-500/30 bg-slate-900 p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-violet-400">🎯 Nuevo objetivo semanal</h3>
              <button onClick={() => setShowGoalForm(false)} className="text-slate-400 hover:text-white"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-sm text-slate-300">Semana *</label>
                <input
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 text-white outline-none focus:border-violet-500"
                  value={goalForm.weekLabel}
                  onChange={(e) => setGoalForm({ ...goalForm, weekLabel: e.target.value })}
                  placeholder="Ej: Semana 1 — 3 al 7 de Febrero"
                />
              </div>
              <div>
                <label className="mb-1 block text-sm text-slate-300">Objetivo *</label>
                <textarea
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 text-white outline-none focus:border-violet-500"
                  rows={4}
                  value={goalForm.content}
                  onChange={(e) => setGoalForm({ ...goalForm, content: e.target.value })}
                  placeholder="Describe el objetivo de esta semana..."
                />
              </div>
              <div>
                <label className="mb-1 block text-sm text-slate-300">Asignar a (opcional — vacío = todos)</label>
                <select
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 text-white outline-none focus:border-violet-500"
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button onClick={() => setShowGoalForm(false)} className="rounded-xl border border-slate-700 px-4 py-2 text-sm text-slate-300">Cancelar</button>
                <button
                  onClick={handleSaveGoal}
                  disabled={savingGoal || !goalForm.weekLabel.trim() || !goalForm.content.trim()}
                  className="rounded-xl bg-violet-500 px-5 py-2 text-sm font-medium text-white hover:bg-violet-400 disabled:opacity-50"
                >
                  {savingGoal ? 'Guardando...' : 'Publicar objetivo'}
                </button>
              </div>
            </div>
          </div>
