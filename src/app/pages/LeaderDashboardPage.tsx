  ...
  async function handleAddComment(isSupport: boolean) {
    ...
    try {
      await addTaskComment({
        taskId:    selectedTeamTask.id,
        authorId:  profile.id,
        comment:   isSupport ? '👋 Apoyé esta tarea' : newComment.trim(),
        isSupport,
      });
        ...
        {([
          ...
          <button
            ...
            className={`flex items-center gap-2 rounded-t-xl px-4 py-2.5 text-sm font-medium transition-all ${
              activeTab === key ? 'border border-b-0 border-slate-700 bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-300'
            }`}
          >
            {icon} {label}
          </button>
                          ...
                          {isExpanded && canInteract && (
                            <div className="mt-4 border-t border-slate-700 pt-4 space-y-3">
                              ...
                              <div className="flex flex-wrap gap-2 pt-1">
                                ...
                                {['pending', 'in_progress'].includes(task.status) && <button disabled={busyId === task.id} onClick={(e) => { e.stopPropagation(); setCompletingTask(task); setCompletionComment(''); }} className="rounded-lg bg-emerald-500 px-3 py-1.5 text-xs font-medium text-slate-950 hover:bg-emerald-400 disabled:opacity-50">✓ Marcar lista</button>}
                                {task.status !== 'blocked' && task.status !== 'completed' && task.status !== 'pending_validation' && <button disabled={busyId === task.id} onClick={(e) => { e.stopPropagation(); handleReportBlock(task.id); }} className="rounded-lg border border-red-500/40 px-3 py-1.5 text-xs text-red-300 hover:bg-red-500/10 disabled:opacity-50">🚨 Reportar bloqueo</button>}
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {allTasks.map((task) => {
              ...
              return (
                <div
                  key={task.id}
                  className={`rounded-2xl border p-4 cursor-pointer hover:border-slate-600 transition-all ${isOwn ? 'border-cyan-500/30 bg-cyan-500/5' : 'border-slate-800 bg-slate-950/50'}`}
                  onClick={() => openTeamTask(task)}
                >
